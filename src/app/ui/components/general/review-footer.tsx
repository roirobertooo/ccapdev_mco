...
function ReviewFooter({review, classOpts}: ReviewFooterProps) {
    ...
    useEffect(() => {
        if (review) {
            ...
            setDislikes(review.dislikes_count);
        }
        if (currentUser) {
            setHasLiked(review.likes.includes(currentUser));
            setHasDisliked(review.dislikes.includes(currentUser));
        }
    }, [currentUser, review]);

    const handleLikeClick: MouseEventHandler<HTMLButtonElement> = (event) => {
        // Optimistically update likes count and like status on client side
        setLikes((prevLikes) => prevLikes ? (hasLiked ? prevLikes - 1 : prevLikes + 1) : (hasLiked ? review.likes_count - 1 : review.likes_count + 1));
        setHasLiked(!hasLiked);

        // Prepare the update values based on whether the user has already liked the review
        ...
        fetch(postString, {
            ...
            .catch(error => {
                console.error(error);
                // Revert likes count and like status on client side if server update fails
                setLikes((prevLikes) => prevLikes ? (hasLiked ? prevLikes + 1 : prevLikes - 1) : (hasLiked ? review.likes_count + 1 : review.likes_count - 1));
                setHasLiked(!hasLiked);
            });
    };

    const handleDislikeClick: MouseEventHandler<HTMLButtonElement> = (event) => {
        // Optimistically update dislikes count and dislike status on client side
        setDislikes((prevDislikes) => prevDislikes ? (hasDisliked ? prevDislikes - 1 : prevDislikes + 1) : (hasDisliked ? review.dislikes_count - 1 : review.dislikes_count + 1));
        setHasDisliked(!hasDisliked);

        // Prepare the update values based on whether the user has already disliked the review
        ...
        fetch(postString, {
            ...
            .catch(error => {
                console.error(error);
                // Revert dislikes count and dislike status on client side if server update fails
                setDislikes((prevDislikes) => prevDislikes ? (hasDisliked ? prevDislikes + 1 : prevDislikes - 1) : (hasDisliked ? review.dislikes_count + 1 : review.dislikes_count - 1));
                setHasDisliked(!hasDisliked);
            });
    };
        ...
        <div className={`flex flex-row items-center justify-around ${classOpts}`}>
            ...
            <div className="flex flex-row justify-end items-center gap-2 ml-auto">
                <button onClick={handleLikeClick}
                        className="flex flex-row border-2 rounded-full px-3 py-1.5 text-sm font-semibold text-gray-700 w-fit items-center gap-1 hover:bg-gray-100 transition ease-linear 0.1s">
                    {hasLiked ?
